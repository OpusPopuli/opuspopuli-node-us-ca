...
# Prometheus textfile metrics (opuspopuli#1217). Deliberately NOT emitted on
# the "prompts_db_unreachable" skip path: prompt-service colocation is
# optional, so on a node without it the series should simply never exist
# rather than alert forever. Once a prompts backup has succeeded once,
# BackupStale governs it like any other database.
# shellcheck source=backup/scripts/metrics.sh
if [[ -r "$(dirname "$0")/metrics.sh" ]]; then
  . "$(dirname "$0")/metrics.sh"
  metrics_init "opuspopuli-prompts-db"
else
